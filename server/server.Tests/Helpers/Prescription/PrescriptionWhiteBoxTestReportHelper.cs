...
{
    /// <summary>
    /// Helper class để xuất báo cáo Unit Test WhiteBox cho chức năng Kê đơn thuốc
    /// Dữ liệu trả về dạng JSON để kiểm tra API response
    /// </summary>
    public class PrescriptionWhiteBoxTestReportHelper
    {
        ...
        public class WhiteBoxTestResult
        {
            ...
            public string ActualResult { get; set; } = "";   // Gom StatusCode + Response
        ...
        {
            Console.WriteLine($"[WhiteBoxTest] {DateTime.Now:HH:mm:ss} - {message}");
        }
        ...
        public static void ExportToExcel(string filePath)
        {
            ...
            {
                var worksheet = workbook.Worksheets.Add("WhiteBox_Prescription");

                // Định nghĩa tiêu đề cột theo format WhiteBox Testing (đã tối giản)
                ...
                // Điều chỉnh độ rộng cột (đã tối giản)
                ...
                try
                {
                    ...
                    Console.WriteLine($"✅ WhiteBox Report saved at: {filePath}");
                }
                catch (IOException)
                {
