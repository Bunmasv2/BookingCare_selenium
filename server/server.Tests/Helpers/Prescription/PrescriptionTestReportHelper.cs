...
{
    /// <summary>
    /// Helper class để xuất báo cáo Unit Test cho chức năng Kê đơn thuốc
    /// Tham khảo cấu trúc từ AppointmentTestReportHelper.cs
    /// </summary>
    public class PrescriptionTestReportHelper
    {
        private static List<PrescriptionTestResult> _results = new List<PrescriptionTestResult>();

        /// <summary>
        /// Cấu trúc kết quả test theo format Excel
        /// </summary>
        public class PrescriptionTestResult
        {
            public string Id { get; set; } = "";
            public string Items { get; set; } = "Kê đơn thuốc";
        ...
        {
            Console.WriteLine($"[PrescriptionTest] {DateTime.Now:HH:mm:ss} - {message}");
        }

        /// <summary>
        /// So sánh Expected và Actual để xác định Pass/Fail - EXACT MATCH
        /// </summary>
        private static bool CompareExpectedActual(string expected, string actual)
        {
            ...
            // So sánh chính xác (exact match) - không normalize
            ...
            bool testPassed)
        {
            ...
            _results.Add(new PrescriptionTestResult
            {
                Id = id,
                Items = "Kê đơn thuốc",
        ...
        public static void ExportToExcel(string filePath)
        {
            ...
            {
                var worksheet = workbook.Worksheets.Add("Prescription_WhiteBox_Test");
                ...
                worksheet.Column(3).Width = 45;   // Description
                worksheet.Column(4).Width = 40;   // PreCondition
                worksheet.Column(5).Width = 50;   // Steps to Execute
                worksheet.Column(6).Width = 45;   // Expected Output
                worksheet.Column(7).Width = 40;   // Test Data
                worksheet.Column(8).Width = 10;   // Edge
                worksheet.Column(9).Width = 10;   // Chrome
                worksheet.Column(10).Width = 45;  // Actual Output
                ...
                catch (IOException)
                {
                    ...
                    var newFilePath = Path.Combine(
                        ...
                        $"WhiteBox_Prescription_Report_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx"
                    );
